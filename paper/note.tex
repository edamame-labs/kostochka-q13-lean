...
The question is natural: by the second feature above, the ``only if'' direction holds,
and for ordinary equitable coloring the independence condition is the only obstruction.
We show that in the list setting the ``if'' direction fails for every $k$, even for
trees, and even for the weaker notion of \cite{KPW}. The failure is by the smallest
possible margin (Remark~\ref{rem:KMW}), which suggests that a modified characterization
may still exist; see Section~\ref{sec:open}.
...
\cite[Proposition~22]{KMW} showed that $K_{1,(k-1)(k^3-k+2)}+K_{1,k^3}$ is not equitably
$k$-choosable; their example was constructed to show that a sufficient condition in
\cite{KMW} cannot be relaxed, and Question~\ref{q13} was posed later. Read in the
present context, that forest also satisfies the independence condition and thus already
gives a negative answer. Our $F_k$ is the smallest member of the same family of examples
(Remark~\ref{rem:KMW}). In Section~\ref{sec:general}
...
precisely the obstruction that makes Theorem~\ref{thm:CL} true; the list version escapes
it by a single vertex, so the independence condition fails to be sufficient by the
...
\bibitem{KMW} H.~Kaul, J.~A.~Mudrock, T.~Wagstrom, On the equitable choosability of the
  disjoint union of stars, \emph{Graphs Combin.} 38 (2022), no.~5, Paper No.~163;
  arXiv:2008.06333. (Result numbers refer to the arXiv version.)
\bibitem{KKXplanar} H.~A.~Kierstead, A.~V.~Kostochka, Z.~Xiang, Equitable list coloring
  of planar graphs with given maximum degree, \emph{J.~Graph Theory} 108 (2025), no.~4,
  832--838.
\bibitem{KKXsparse} H.~A.~Kierstead, A.~V.~Kostochka, Z.~Xiang, Equitable list coloring
  of sparse graphs, \emph{Discrete Math.} 349 (2026), no.~5, Paper No.~114981;
  arXiv:2411.08372.
\bibitem{KKXsurvey} H.~A.~Kierstead, A.~V.~Kostochka, Z.~Xiang, Results and problems on
  equitable coloring of graphs, in: G.~O.~H.~Katona et al.\ (eds.), \emph{Sum(m)it 280:
  Surveys in Extremal Combinatorics and Combinatorial Geometry}, Bolyai Soc.\ Math.\
  Stud.\ 32, Springer, 2026, pp.~231--260; arXiv:2504.14711. (Question and theorem
